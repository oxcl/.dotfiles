...
3.  **Dispatch the Prompt in parallel:** Use the `handoff_write` tool with the user's exact prompt as the argument. You will receive a reference ID back. Pass **only this reference ID** to each selected expert via the task tool — do NOT pass the full prompt text. Each expert will use `handoff_read` to retrieve the content.
