...
#+begin_src elisp
  ;; this function automatically adds "http://" to current request url if it lacks one
  (defun oxcl/inject-http-to-url ()
    ...
    (let ((line (buffer-substring-no-properties (line-beginning-position) (line-end-position))))
      (when (and ;; if line begins with a http method but it doesn't include a http:// or https://
	   (string-match "^\\(GET\\|POST\\|PUT\\|PATCH\\|DELETE\\) .*$" line)
  ...
  (use-package restclient
    ...
    :config
    (require 'restclient-jq) ; enable jq integration
    ;; disable the builtin folding functionality
    ;;(remove-hook 'restclient-mode-hook 'restclient-outline-mode)
    
    (setq restclient-inhibit-cookies t
	  restclient-content-type-modes (push '("application/json" . json-ts-mode)
					      restclient-content-type-modes)))
#+end_src
** jq integration
use jq to query through the response object and store them in restclient variables with -> jq-set-var hook
#+begin_src elisp
    (use-package jq-mode :defer)
    (use-package restclient-jq :defer)
#+end_src
#+begin_src elisp
#+end_src
