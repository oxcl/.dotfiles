...
* Packages
before loading any third party packages we first check the two variables ~oxcl/load-essentials-only~ and ~oxcl/load-elpaca-only~ and if any of them is true we will not load any 3rd party packages.
with this you can easily the configuration of emacs to allow easier debugging and testing new packages.
#+begin_src elisp
