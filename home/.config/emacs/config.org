...
** font
#+begin_src emacs-lisp
  (set-frame-font "JetBrains Mono 8" t t)
  (setq-default line-spacing 1) ; small padding between each line
#+end_src

** consistent font size across monitors
#+begin_src elisp
  ;; Font size adjustment
(defun oxcl/adjust-font-size (frame)
...
(add-hook 'window-size-change-functions #'oxcl/adjust-font-size)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (add-hook 'restclient-mode-hook (lambda () (setq-local global-hl-line-mode nil)))
...
#+begin_src elisp
  ...
  (defun oxcl/setup-auto-insert ()
	...
	(dolist (item auto-insert-files)
      ...
      ;; this snippet adds an extra new line to every new file that is opened.
      ;; fixes the problems with display-line-number-mode and hl-line-mode
      (add-to-list 'auto-insert-alist `("\\.*" . ["_" oxcl/yas-insert]))))
